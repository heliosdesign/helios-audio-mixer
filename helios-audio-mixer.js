/**************************************************************************
    ...
    Web Audio API, fallback to HTML5
    ...
    ~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        ~~~
            ...
            updateTween() -> IMPORTANT: add this to your RAF function for tweens to work
    ...
    ~~~~~~~~~~~~~
        ...
        nodes         []        array of strings: names of desired additional audio nodes
                                (TODO: some add'l node require parameters to be set)
...
;(function(window, undefined){
    ...
    **************************************************************************/
    ...
    Mix.prototype.pause = function(){
        ...
        log('[Mixer] Pausing all '+total+' track(s) ||', 1)
    ...
    Mix.prototype.play = function(){
        var total = this.tracks.length;
        log('[Mixer] Playing all '+total+' track(s) >', 1)
    ...
    Mix.prototype.stop = function(){
        var total = this.tracks.length;
        log('[Mixer] Stopping all '+total+' track(s)', 1)
    ...
    **************************************************************************/
    ...
    Track.prototype.loadDOM = function( source ){
        ...
        self.element.addEventListener('canplaythrough', function(e) {
            ...
            self.ready = true;
            self.mix.trigger('load', self);

            if(self.options.autoplay) self.play();

            if(self.options.loadCallback) {
                if(typeof self.options.loadCallback === 'function') self.options.loadCallback();
            }
        }, false);
    ...
    **************************************************************************/
    ...
    Track.prototype.play = function(){
        ...
        if(!Detect.webAudio) {
            ...
            var startFrom = self.options.currentTime || 0;
            log('[Mixer] Playing track "'+self.name+'" from '+startFrom, 1)
    ...
    Track.prototype.pause = function(){
        ...
        var doIt = function(){
            ...
            if(!Detect.webAudio) {
                self.options.currentTime = self.element.currentTime;
                self.element.pause();
            } else {

                // cache currentTime to resume from later
                self.options.currentTime = self.options.source.context.currentTime;
            ...
            log('[Mixer] Pausing track "'+self.name+'" at '+self.options.currentTime,1)
